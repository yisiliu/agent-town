...
export default function GangPanel({
  ...
  const myGangs = useQuery(getPlayerGangsRef, playerId ? { playerId } : 'skip') as PlayerGangEntry[] | undefined;
  const allPlayers = useQuery(listAllPlayersWithGangStatusRef, { worldId }) as PlayerWithGangStatus[] | undefined;
  const pendingInvites = useQuery(getPendingInvitesRef, playerId ? { playerId } : 'skip') as PendingInvite[] | undefined;
  ...
  const handleCreateGang = async () => {
    ...
    if (!playerId) {
      console.log('[GangPanel] No player available for creating gang');
      return;
    }
    try {
      await createGang({
  ...
  const handleJoinGang = async (gangId: string) => {
    if (!playerId) {
      console.log('[GangPanel] No player available for joining gang');
      return;
    }
    try {
      await joinGang({ gangId, playerId });
    } catch (e) {
  ...
  const handleInvitePlayer = async (inviteeId: string) => {
    if (!playerId || !selectedGangForInvite) return;
    try {
      await inviteToGang({
  ...
  const handleAcceptInvite = async (inviteId: string) => {
    if (!playerId) return;
    try {
      await acceptInvite({ inviteId, playerId });
    } catch (e) {
  ...
  const handleRejectInvite = async (inviteId: string) => {
    if (!playerId) return;
    try {
      await rejectInvite({ inviteId, playerId });
    } catch (e) {
  ...
  return (
    <div className="flex flex-col h-full" style={{ backgroundColor: COLORS.bg, color: COLORS.text, fontFamily: 'monospace' }}>
      ...
      <div
        className="p-2 border-b-2 transition-all duration-200"
        style={{
      ...
      >
        <h3
          className="text-center text-sm tracking-wider transition-all duration-200"
          style={{
