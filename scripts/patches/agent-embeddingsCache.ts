...
import { fetchMinimaxBatch } from '../ours/lib/minimaxEmbedding';

// Route embeddings through MiniMax embo-01 (1536-dim, Chinese-native)
// when MINIMAX_EMBEDDINGS_ENABLED=1; fall back to ai-town's Together-
// backed fetchEmbeddingBatch otherwise. The gate is explicit (not just
// "is MINIMAX_API_KEY set") because the schema's EMBEDDING_DIMENSION
// must match the active provider — flipping providers without the
// matching schema bump will crash vector-index writes.
async function fetchEmbeddings(texts: string[]) {
  if (process.env.MINIMAX_EMBEDDINGS_ENABLED === '1') {
    return fetchMinimaxBatch(texts);
  }
...
export async function fetchBatch(ctx: ActionCtx, texts: string[]) {
  ...
  if (cacheResults.length < texts.length) {
    ...
    const response = await fetchEmbeddings(missingTexts);
