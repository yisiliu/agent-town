...
// MINIMAX_EMBEDDINGS_ENABLED=1 so flipping providers requires an
// explicit schema-dimension bump in lockstep (see that file's note).
async function embedOne(text: string) {
  if (process.env.MINIMAX_EMBEDDINGS_ENABLED === '1') {
    return fetchMinimaxSingle(text);
  }
  return fetchEmbedding(text);
}

// Mirror of agent/conversation.ts townChat — route memory summaries,
...
export async function rememberConversation(
  ...
  // The conversation can vanish between the agent deciding to remember it
  // and this action firing — the engine archives + GCs conversations on its
  // own cadence. Swallow the not-found race quietly; the agent's "remember"
  // op succeeds as a no-op rather than crashing the engine's runStep.
  let data;
  try {
    ...
    if ((e as Error).message?.includes('not found')) {
      console.debug(`rememberConversation: ${conversationId} already gone, skipping`);
      return;
    }
    throw e;
  }
...
async function calculateImportance(description: string) {
  const { content: importanceRaw } = await townChat({
    ...
    // V4 Pro burns reasoning tokens before emitting content; <100 starves
    // out the digit. We trim to the first numeric character downstream.
    max_tokens: 200,
  });
