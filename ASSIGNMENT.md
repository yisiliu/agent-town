...
**Q：issue 顶部写"依赖 #X"，但 #X 还没人做怎么办？**
答：两条路：(a) 先做 #X 再做你认领的，复杂度高、不推荐；(b) **mock 掉依赖**——比如你做"上班赚工资"（#11）依赖"货币系统"（#5）但 #5 没做，可以在你的 PR 里写一个"假"货币 mutation（直接 patch player.coins，跳过 input 流），然后**在披露段写明你 mock 了什么、为什么、未来 #5 实现后该怎么接**。后者是工程现实，老师认可。
