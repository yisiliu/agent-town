...
export class Conversation {
  ...
  acceptInvite(game: Game, player: Player) {
    ...
    // Idempotent: between the UI showing the invite and the click
    // reaching the backend, the engine may already have transitioned
    // the membership past 'invited' (e.g. to 'walkingOver' when the
    // inviter is approaching). Treat a click on the already-accepted
    // state as a no-op rather than throwing.
    if (member.status.kind === 'walkingOver' || member.status.kind === 'participating') {
      return;
    }
